...
async def proxy_discover():
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{FASTAPI_URL}/fastapi/api/edf-files/discover")
        return resp.json()
...
async def proxy_apply(file_name: str, request: Request):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{FASTAPI_URL}/fastapi/api/edf-files/{file_name}/apply", json=payload)
